...
def api_login_required(func):
    ...
    return decorated
#================== Subcategory List ===========================
sub_category_list_field = { 'id': fields.Integer,
                        'name': fields.String,
                        'logo_path': fields.String,
                        'logo': fields.String,
                    }
class SubcategoryList(Resource):
    @marshal_with(sub_category_list_field)
    def get(self, id):
        return models.Categories.query.filter_by(parent_id=id).order_by(models.Categories.name.asc()).all()
api = Api(app)
api.add_resource(SubcategoryList, '/api/subcategory/list/<int:id>', endpoint='api_subcategory_list')
